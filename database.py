...
def init_db():
    try:
        ...
        if DATABASE_URL:
            ...
            # --- NUEVA TABLA: ÁRBITRO DE CAPTURAS (Postgres) ---
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS spawns_activos (
            ...
            ''')
            log.info("✅ Tabla 'spawns_activos' creada/verificada en PostgreSQL")
            ...
            # --- NUEVA TABLA: ÁRBITRO DE CAPTURAS (SQLite) ---
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS spawns_activos (
            ...
            ''')
            log.info("✅ Tabla 'spawns_activos' creada/verificada en SQLite")
...
def obtener_lista_capturas(user_id):
    ...
    try:
        ...
        if conn:
            conn.close()
def registrar_spawn_db(channel_id, nombre_pokemon):
    """
    Registra un nuevo Pokémon en el canal. 
    Limpia cualquier estado previo para asegurar un inicio limpio.
    """
    conn = get_connection()
    try:
        with conn:
            with conn.cursor() as cur:
                # Usamos INSERT con ON CONFLICT para PostgreSQL
                # o REPLACE para SQLite.
                if DATABASE_URL:
                    cur.execute('''
                        INSERT INTO spawns_activos (channel_id, pokemon_nombre, estado)
                        VALUES (%s, %s, 'libre')
                        ON CONFLICT (channel_id) DO UPDATE 
                        SET estado = 'libre', pokemon_nombre = EXCLUDED.pokemon_nombre
                    ''', (channel_id, nombre_pokemon))
                else:
                    cur.execute('''
                        REPLACE INTO spawns_activos (channel_id, pokemon_nombre, estado)
                        VALUES (?, ?, 'libre')
                    ''', (channel_id, nombre_pokemon))
        log.info(f"📍 Spawn registrado en canal {channel_id}: {nombre_pokemon}")
    finally:
        conn.close()

def intentar_reclamar_spawn_db(channel_id, user_id):
    """
    Intenta marcar el spawn como 'capturado' de forma atómica.
    Retorna True si el usuario fue el primero, False si ya estaba capturado.
    """
    conn = get_connection()
    try:
        with conn:
            with conn.cursor() as cur:
                # La consulta clave: Solo actualiza si el estado es 'libre'
                if DATABASE_URL:
                    cur.execute('''
                        UPDATE spawns_activos 
                        SET estado = 'capturado', capturador_id = %s 
                        WHERE channel_id = %s AND estado = 'libre'
                    ''', (str(user_id), channel_id))
                else:
                    cur.execute('''
                        UPDATE spawns_activos 
                        SET estado = 'capturado', capturador_id = ? 
                        WHERE channel_id = ? AND estado = 'libre'
                    ''', (str(user_id), channel_id))
                
                # rowcount nos dice si se actualizó una fila
                return cur.rowcount == 1
    finally:
        conn.close()
def limpiar_spawn_db(channel_id):
    """Limpia el registro del spawn en el canal para permitir futuros spawns."""
    conn = get_connection()
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM spawns_activos WHERE channel_id = %s", (channel_id,))
        log.info(f"🧹 Spawn limpiado en canal {channel_id}")
    finally:
        conn.close()