...
class BotonCaptura(discord.ui.View):
    ...
    async def on_timeout(self):
            # 1. Limpiamos en la Base de Datos (en lugar de gestor_spawn)
            database.limpiar_spawn_db(self.message.channel.id)
    ...
    async def boton_captura(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            # 1. Filtro de Cooldown (Mantenido)
            ...
            # 2. Defer inmediato (Fundamental para evitar error 10062)
            await interaction.response.defer(ephemeral=True)

            # 3. ATOMICIDAD: Bloqueo en BD
            # Si esto retorna False, significa que alguien más ya atrapó el Pokémon
            if not database.intentar_reclamar_spawn_db(interaction.channel.id, user_id):
                return await interaction.followup.send("💨 ¡Llegaste tarde! Alguien más fue más rápido.", ephemeral=True)

            self.user_cooldowns[user_id] = ahora

            try:
                # 4. Probabilidades (Tu lógica original)
                azar = random.random()
                ...
                if random.random() < prob_final:
                    ...
                    # Limpiamos el spawn de la BD tras el éxito
                    database.limpiar_spawn_db(interaction.channel.id)
                    ...
                    if self.intentos >= self.max_intentos:
                        ...
                        # Limpiamos el spawn si huye
                        database.limpiar_spawn_db(interaction.channel.id)
                        await interaction.message.edit(content="💨 ¡El Pokémon escapó tras fallar demasiadas veces!", view=None)
                        self.stop()
                    else:
                        # En caso de fallo, liberamos el bloqueo para que otros (o el mismo usuario) puedan intentar de nuevo
                        # Nota: Requiere una función pequeña en database.py para resetear el estado a 'libre'
                        # Si no quieres que otros atrapen el mismo tras fallar, simplemente omite esta parte.
                ...
                try:
                    msg = "⚠️ Ocurrió un error interno al intentar atrapar al Pokémon."
