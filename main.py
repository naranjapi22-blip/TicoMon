...
async def spawn(ctx):
    ...
    # 3. PASO 4: REGISTRO EN BASE DE DATOS (En lugar de set en memoria)
    # Registramos que hay un encuentro activo en este canal
    database.registrar_spawn_db(ctx.channel.id, "Spawn Seleccionable")

    try:
        # ... (Tu lógica de generación de Pokémon sigue IGUAL) ...
        ids_spawn = []
        ...
        if not buffer_siluetas:
            # Limpiamos si algo falla
            database.limpiar_spawn_db(ctx.channel.id)
        ...
        database.limpiar_spawn_db(ctx.channel.id)
        database.actualizar_energia_db(ctx.author.id, intentos, ultima_recarga)
        log.error(f"Error en spawn: {e}", exc_info=True)
